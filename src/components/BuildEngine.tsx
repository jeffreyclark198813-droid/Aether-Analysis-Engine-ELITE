...
const BuildEngine: React.FC = () => {
  ...
  const runBuild = useCallback(async () => {
    ...
    addLog(`Build initiated for ${selectedProfile.name}`);
    ...
    for (let i = 0; i < BUILD_STEPS.length; i++) {
      ...
      const progress = ((i + 1) / BUILD_STEPS.length) * 100;
    ...
    addLog(
      'SUCCESS: The build engine has finished building the full, true, complete, patched, modded, and rebuilt version.'
    );
    addLog(`Build completed successfully in ${duration}s`);
    addLog('Artifact: app-release-signed.apk generated.');
  }, [selectedProfile, isBuilding, addLog]);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full overflow-hidden">
              ...
              <div className="bg-zinc-950 border border-zinc-800 p-4 rounded-xl flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-bold text-white truncate">
                    {selectedProfile?.name || 'No Artifact Selected'}
                  </p>
                  <p className="text-[10px] font-mono text-zinc-500 truncate">
                    {selectedProfile?.packageName || '---'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-8 flex gap-2">
            ...
            {isBuilding && (
              <button
                ...
                className="px-4 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white"
              >
                Stop
              </button>
            )}
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-4">
            {BUILD_STEPS.map((step, i) => {
              const isCompleted =
                buildProgress > ((i + 1) / BUILD_STEPS.length) * 100 ||
                buildStatus === 'success';
      ...
      <div className="lg:col-span-2 flex flex-col gap-8 overflow-hidden">
        <div className="glass-panel p-8 relative overflow-hidden">
          <motion.div
            className="absolute top-0 left-0 h-1 bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.5)]"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-zinc-950/50 border border-zinc-900 p-6 rounded-2xl flex flex-col items-center text-center">
              ...
              <span className="text-[10px] font-mono text-zinc-500 uppercase mb-1">
                Output
              </span>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono text-emerald-500/70 uppercase">
                    Ready for deployment • 42.5 MB
                  </p>
                </div>
              </div>
